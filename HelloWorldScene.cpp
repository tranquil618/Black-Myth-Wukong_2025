...
#define PALADIN_MODEL "Maria.c3b" // 确保文件名正确
...
{
    _player = Maria::create(PALADIN_MODEL);
    if (!_player) {
...
bool HelloWorld::setupGoblin() {
    _goblin = EnemyGoblin::create();
    if (_goblin) {
        // 假设玩家在 (0,0,0)，把哥布林放在 (100, 0, 100)
        _goblin->setPosition3D(Vec3(100.0f, 0.0f, 100.0f));
        this->addChild(_goblin);
        return true;
    }
    return false;
}
...
void HelloWorld::setupInputListeners()
{
    ...
    keyboardListener->onKeyPressed = [this](EventKeyboard::KeyCode keyCode, Event* event) {
        ...
        // 3. 键位修改：空格键闪避
            ...
            // 获取当前方向键状态作为闪避方向
            Vec3 dir = Vec3::ZERO;
            if (_keys[EventKeyboard::KeyCode::KEY_W]) dir.z += 1;
            if (_keys[EventKeyboard::KeyCode::KEY_S]) dir.z -= 1;
            if (_keys[EventKeyboard::KeyCode::KEY_A]) dir.x += 1;
            if (_keys[EventKeyboard::KeyCode::KEY_D]) dir.x -= 1;
    ...
    mouseListener->onMouseMove = [this](Event* event) {
        ...
        this->_player->setCameraYawAngle(this->_currentCameraYaw);
...
bool HelloWorld::init()
{
    ...
    setupCamera();
    setupMaria();   // 创建 _player (Maria)
    if(!setupGoblin()) return false;  // 创建 _goblin (EnemyGoblin)
    // --- 关键修改：建立战斗目标关联 ---
    if (_goblin && _player) {
        _goblin->setTarget(_player);
        // 这一步会让 Goblin 的 update 开始执行追击和攻击逻辑
    }
...
void HelloWorld::update(float dt)
{
    ...
    bool isRunning = isMoving && _keys[EventKeyboard::KeyCode::KEY_SHIFT];

    // ================= 移动指令 =================
    if (isMoving)
    {
        direction.normalize();
        _player->runMove(direction, isRunning);
    }
    else
    {
        _player->stopMove();
